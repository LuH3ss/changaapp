        ...
        <Box style={stylesJP.container}>
            {
                ...
                : revSlice?.map(e => {
                    return( 
                        <Box key={e.id} sx={{display:'flex', border:'solid grey 1px', borderRadius:'10px', padding:'2%', margin:'2%', justifyContent:'space-between', width:'80%'}}>
                            <Box sx={{display:'flex', flexDirection:'column', alignItems:'center'}}>
                                <img style={{width:'100px'}} src={e.author?.img ? e.author?.img : userImg} alt="Rompio" width='100px' height='100px' />
                            </Box>
                            <Box sx={{display:'flex', flexDirection:'column', justifyContent:'space-between'}}>
                            <Typography sx={{fontSize:'1.2rem'}}>Autor: {e.author?.firstName} {e.author?.lastName}</Typography>
                            <Typography sx={{fontSize:'1.2rem', overflow:'auto'}}>Mensaje: {e.message}</Typography>
                            <Typography sx={{display:'flex', fontSize:'1.2rem', alignItems:'center'}}>Calificacion: <Rating
                            defaultValue={e.rate} 
                            readOnly
                            /></Typography>
                            </Box>
